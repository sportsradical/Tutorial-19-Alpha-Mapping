...
bool AlphaMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../Engine/source/alphamap.vs", L"../Engine/source/alphamap.ps");
	if(!result)
	{
		return false;
	}
...
bool AlphaMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, 
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView** difuse1, ID3D11ShaderResourceView** difuse2, ID3D11ShaderResourceView** alpha)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, difuse1, difuse2, alpha);
	if(!result)
	{
		return false;
	}
...
bool AlphaMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = D3DCompileFromFile(vsFilename, NULL, NULL, "AlphaMapVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 
...
bool AlphaMapShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix,
											  ...
											  ID3D11ShaderResourceView** difuse1, ID3D11ShaderResourceView** difuse2, ID3D11ShaderResourceView** alpha)
{
	HRESULT result;
	...
	deviceContext->PSSetShaderResources(0, 1, difuse1);
	deviceContext->PSSetShaderResources(1, 1, difuse2);
	deviceContext->PSSetShaderResources(1, 1, alpha);
